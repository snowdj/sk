...
$$Max: R = \sum_{k=1}^n ( p_k x_k y_k - a_k x_k - b_k  x_k^2)$$  
...
- Based on these cost functions $a_k x_k - b_k  x_k^2$, if we want to evaluate the impact of policy,need to look at $p_k x_k y_k$
     ...
     - Monte Carlo simulation for the price $p_k$ and yield $y_k$. (Turvey, 2012).
      ...
      - $P_{i0}$ is the initial spring price as of March 2014; price generation is based on a 7-month growing season; $P_{ij}$ is the random commodity harvest price generated by a log normal (Brownian) process
...
- Since yield data is a time series, in order to find the __$\sigma$__ of yield, we need to detrend the yield data.(Coble, 2013).
 ...
 - For example, the left graph is time trend for Wheat yield; the right is the residual.
...
---
## Yield Simulation Based on 5 Year Mean and Detrand Standard Deviation

<!-- html table generated in R 3.0.3 by xtable 1.7-3 package -->
<!-- Fri Aug 15 17:58:42 2014 -->
<TABLE border=1>
<TR> <TH>  </TH> <TH> Wheat </TH> <TH> Oats </TH> <TH> Flax </TH> <TH> Flax.1 </TH> <TH> Wheat.1 </TH>  </TR>
...
$$P_{ij} = P_{i0} e^{((\mu - \frac{1}{2}) \frac{7}{12} + \sigma N(0,1) \sqrt{\frac{7}{12}})}$$
      ...
      - $P_{ij}$ is the random commodity harvest price generated by a log normal (Brownian) process
       - with drift $\mu$, 
       - volatility $\sigma$, and random deviate drawn from a normal distribution with zero mean and variance of 1.0;
...
$\mu$ is **drift**; $\sigma$ is **diffusion**.
...
<!-- html table generated in R 3.0.3 by xtable 1.7-3 package -->
<!-- Fri Aug 15 17:58:44 2014 -->
<TABLE border=1>
<CAPTION ALIGN="bottom"> Estimated Mu and Sigma based on historical data </CAPTION>
...
---
## Conclusion
- With the cost, yield, and price data, we can structure our model.For example, in terms of stable farmer's income risk, the farm model can be structured as follows:

$$Min: \sigma_p^2 = \frac{1}{m} \sum_{k=1}^m ( \pi_j - E[\pi] )^2$$
$$Subjet \; to: \sum_{i=1}^n x_k = 1668 \; $$ 
$$x_k \ge 0$$
$$\sum_{i=1}^n E[R_i] x_i = K \; (2)$$

- Where $\sigma_p^2$ measure the unstability of farmers' income. 
- (2) $K$ is the target profit, $R$ is the average profit per acre.
...
---
## Conclusion
- Based on this, we can add plocily constraints to see if farmers risk change under the prolicy. For example, add a whole farm insurance constraints with $m=1000$ times simulation state.
...
- Where $Z$ is 70% farmers history income
- $Max[Z - \sum_{i=1}^n R_{1,i} x_i, \; 0]$ is what farmer can get from insurance.
...
- Solve the  problem in GAMS $\to$ we can tell the impact of the whole farm insurance.  
